...
namespace MONGOWebAPI.Models
{
    public class MemberController : ApiController
    {
        [Route("api/member2")]
        [HttpPost]
        public AddResultResponse Post(AddResultResponse request)
        {
            var result = 0;
            var response = new AddResultResponse();
            string path = @"Json File";
            //var jsonContent = File.ReadAllText(path);
            //JObject data = (JObject)JsonConvert.DeserializeObject(jsonContent);
            JObject data = (JObject)JsonConvert.DeserializeObject("C:\\ConfiguratorData\\DB\\DataChecked.json");

            //JObject myObj = (JObject)JsonConvert.DeserializeObject(Application.StartupPath + "\\jsonfile.json");
            foreach (var resource in data["results"])
            {
            ...
            else
            {
                ...
                位 */
                response.oK = false;
                response.errMsg = "編號為" + "的會員已存在，請重新輸入別組會員編號。";
            }
            return response;
        }
